...
def main(calling, data_path, output_dir, task_num, data_type, revise=False):
    ...
    print(f"[DEBUG] Loading MCTS tree from: {data_path}")
    root = ExtendedMCTS.load(data_path)
    if root.value <= 0:
        print(f"[WARNING] Root value is {root.value}, skipping processing.")
        return
    else:
        print(f"[DEBUG] Root loaded successfully, value={root.value}")
    ...
    for high_path, low_path in tqdm(paired_paths, desc="Processing pairs"):
        ...
        write_to_jsonl(output_dir, output_entry)
        print(f"[DEBUG] Finished processing file: {data_path}")
